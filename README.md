...
| 插件 | 功能 |
|------|------|
| **XRayUnlocker** | `!x` 透视 / `!god` 无敌 / `!st` 修改暗金计数 / `!nf` 防闪 / `!sc` 无限蹲起 / `!wp` 全图可穿 / `!wd` 穿墙无衰减 / `!cs` 自动急停 / `!sp` 锁定空手移速 / `!mb` 魔法子弹 / `!dx` 掉落物透视 / `!cx` C4透视 / `!inv` 隐身 / `!fb` 秒下包秒拆弹 / `!inf` 无限烟火 / `!stattrak` 暗金持久化 / BOT数量解锁 |
...
| `!sp` / `sp 1/0` | 开关锁定空手移速（基础移速锁为 250 u/s，无论手持何种武器/开镜/投掷） |
...
| `!inf` / `inf 1/0` | 开关无限烟火（本人丢出的烟雾弹永不消散、燃烧弹/火永不熄灭；中途关闭则按正常时长消散，来得及重新打开可继续维持） |
